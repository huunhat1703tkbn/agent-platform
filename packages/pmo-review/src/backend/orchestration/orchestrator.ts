...
function instructionsText(): string {
  return [
    ...
    '',
    'CAPACITY / BOTTLENECK — when the user asks WHICH role is at peak / over-allocated, where the',
    'resource crunch is, or for the capacity breakdown by role ("which role is peaking", "where is',
    'the bottleneck", "show capacity by role"): call pmo_capacityGap(planId) and report the',
    'bottleneck role + its projected peak %, and any other roles over capacity.',
    '',
    'WHAT-IF / HIRING — for a hypothetical staffing change ("what if we add 2 ML Engineers to',
    'PLAN-002", "what happens if we lose a backend dev"): call pmo_simulateHeadcount(planId, role,',
    'delta) ONCE (delta is + to add, − to remove) and narrate before→after: the Resource RAG, the',
    'verdict, and — IMPORTANTLY — call out honestly when the verdict does NOT change and WHY (other',
    'dimensions still bind). For "how many people do we need to hire" / "what would make it feasible',
    'on resources": call pmo_recommendHiring(planId) and report the hires for the bottleneck role,',
