...
export default async function (): Promise<() => Promise<void>> {
  const TEMPLATE = 'platform_template_pmo_review';
  handle = await startPgContainer();
  await ensureTemplateDb(handle, TEMPLATE);

  const pool = new Pool({ connectionString: `${handle.baseUrl}/${TEMPLATE}` });
  try {
    // The orchestrator composes the pmo engine: its sub-agents read pmo data and
    // saveReviewReport writes through core.events (the transactional outbox).
    await runMigrations({
      pool,
      modules: [
        { name: 'core', dir: resolve(__dirname, '../../core/drizzle/migrations') },
        { name: 'pmo', dir: resolve(__dirname, '../../pmo/drizzle/migrations') },
      ],
    });
  } finally {
    await pool.end();
