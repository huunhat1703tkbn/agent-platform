...
**Seven shared packages (D13/D14).** Three are load-bearing and land at M1: `shared/db` (three pools per D10/§K.2), `shared/rbac` (VisibilityGate predicate + permission-string types), `shared/testing` (testcontainers + fakes). Three land with their first consumer: `shared/observability` (M2), `shared/crypto` (M2-late per D37 — envelope encryption only; Secrets Manager reader + JWT rotation + HIBP join when later consumers arrive), `shared/mailer` (M2-late, depends on `shared/crypto`). One stays a Phase A seam only: `shared/storage` (S3 wrapper) — declared but unused until Phase B attachments.
...
| D24 | M2 Stream A split into A1 (local-password) + A2 (Entra OIDC). Both required for M2 Stream A demo gate. Rationale: scope tractability; A1 sets forward-compat seams A2 builds on. Reversal: both slices must close before M2 is considered done. | docs/superpowers/plans/2026-05-19-identity-a1.md |
| D25 | No internal contract versioning. Event payloads / TS types mutate in place; `event_version` column stays as a forward hook but never bumps. External URL versioning (`/api/v1/...`) unchanged. Rationale: single-deploy modular monolith; in-process subscribers; no external consumers. Reversal: first external system (other process, public webhook) consuming events — start versioning then. | docs/superpowers/plans/2026-05-19-identity-a1.md |
| D26 | Provider-subject mapping lives in better-auth's `identity.account` (`providerId`/`accountId`). Per-tenant SSO config lives in `identity.tenant_sso_providers` row-per-provider. Rationale: `requirements.md §3.2` mandates additional providers (Google, Okta, generic SAML/OIDC) without schema change. Reversal: if a provider's metadata cannot fit the `account` row + jsonb config, revisit. | docs/superpowers/plans/2026-05-19-identity-a1.md |
| D27 | Admin/CLI-created users skip better-auth's `signUp.emailAndPassword`. Compose `user`/`account`/`user_profile`/`role_grant` rows inside `withEmit()` directly. Rationale: better-auth's signUp wants email verification (mailer-dependent) + session cookie; both wrong for admin/CLI. Reversal: when `shared/mailer` lands (M3) and we add public sign-up — revisit. | docs/superpowers/plans/2026-05-19-identity-a1.md |
| D28 | HIBP outage allows password set/change with logged warning + OTel counter. Rationale: blocking on HIBP availability reduces production-safety, not increases it. Reversal: outage rate >1%/quarter → cache k-anonymity responses or ship a local breach list. | docs/superpowers/plans/2026-05-19-identity-a1.md |
| D29 | M2 Stream A · A2: JIT removed; admin pre-provisioning + account linking only. Rationale: tighter admin control over access; user-instructed. Deviates from `requirements.md §3.4`. Reversal: re-enable JIT only if a customer pushes back hard and we ship a follow-up "JIT mode" toggle per tenant. | docs/superpowers/plans/2026-05-20-identity-a2.md |
| D30 | Single multi-tenant Entra app registration owned by Seta operator. Rationale: lowest-friction operator UX; data isolation via `tenant_id` + `tid` validation, not via app registration. Reversal: per-tenant own Entra app deferred to v1.x for high-trust enterprise tenants needing credential isolation. | docs/superpowers/plans/2026-05-20-identity-a2.md |
| D31 | Email domain ownership verified transitively via Microsoft Graph `Domain.Read.All`. Rationale: inherit Entra's existing DNS verification; no DNS plumbing on Seta side. Reversal: add DNS TXT verification later if Microsoft Graph ever stops exposing isVerified or rate-limits. | docs/superpowers/plans/2026-05-20-identity-a2.md |
| D32 | Admin consent flow is two-step Connect→Consent→Enable. `consent_granted_by_*` populated lazily. Rationale: Microsoft's admin-consent endpoint does not return user claims; two-step is the production-grade norm. Reversal: eagerly capture on a future Microsoft endpoint update. | docs/superpowers/plans/2026-05-20-identity-a2.md |
| D33 | IdP group→role mapping deferred to Phase B "SSO group sync" slice. Rationale: A2 ships pure auth + linking; role grants stay admin-driven. Reversal: triggered when a customer with strict IdP discipline asks for it. | docs/superpowers/plans/2026-05-20-identity-a2.md |
| D34 | Single image, mode-selectable runtime. The "one runtime binary" framing in `architecture.md §A` is amended to "single image, mode-selectable runtime; defaults to all-in-one." `SETA_MODULES` env var selects loaded modules at startup. Cross-module sync calls route through a dispatch shim (`packages/core/src/rpc/`) that picks in-process call vs Hono RPC client based on the loaded-module set. Event bus and boundary discipline unchanged. Rationale: enables independent per-module scaling on AWS ECS Fargate without forking the codebase into microservices; preserves single-container OSS self-host as the default. Reversal: nothing reverses. Supersedes the implicit single-process assumption in §A. | docs/superpowers/specs/2026-05-20-deployment-strategy-design.md |
| D37 | `shared/crypto` envelope format: AES-256-GCM, per-message DEK, JCE-style JSON blob `{ v, alg, kid, wdk, iv, ct, tag }` with `v=1`. Two `KeyProvider` impls (`KmsKeyProvider`, `EnvKeyProvider`); `CRYPTO_KEY_PROVIDER` env selects. Pulled forward from M3 to M2-late because `shared/mailer` depends on it. `selfTest()` at boot crash-loops the process on misconfig. Format supports rotation (`kid` per blob) but does not execute it; active re-encrypt remains Phase B per D11c. | docs/superpowers/specs/2026-05-20-shared-crypto-design.md |
| D36 | `shared/mailer` adopts tenant-first / operator-fallback transport resolution. Per-tenant config (Graph or SMTP) in `integrations.mail_transport_config`; operator default (SMTP-via-nodemailer or dev-stub) in env. Templates centralised in `packages/shared/mailer` (`react-email` TSX). Send queued via graphile-worker; `core.outgoing_emails` is outbox + dedupe + audit. Per-tenant SMTP passwords encrypted via `shared/crypto`. Graph reuses the operator's existing multi-tenant Entra app — adds `Mail.Send` (application) and requires Entra SSO configured per tenant + Application Access Policy documented. Delta from D13/§J.9. | docs/superpowers/specs/2026-05-20-shared-mailer-design.md |
| D27-rev | Reversal of D27: admin/CLI user creation can now trigger an invite email via `shared/mailer` when a `CreateUserInviteOpts` is supplied. The invite link doubles as proof of email control for pre-provisioned users; explicit verification still skipped for admin-created accounts. Existing call sites unchanged — invite is opt-in by the caller. | docs/superpowers/specs/2026-05-20-shared-mailer-design.md |
| D38 | M2 Stream B reshape: split into B1 (foundation), B2 (web UI), B3 (MS connect), B4 (two-way sync). Mastra tools + `planner.agent` deferred to post-planner phase; Stream C (copilot runtime) consequently also deferred. Rationale: user direction to clone MS Planner experience with two-way sync as M2's primary planner deliverable; scope too large for single spec/PR. Reversal: re-engage Stream C if customer commitment requires the agent demo before planner program closes. (Recorded as D34 in the source spec; renumbered here to follow the existing ledger sequence — D34 was claimed by the deployment-strategy ADR landed earlier, D35–D37 by the crypto/mailer ADRs landed since.) | docs/superpowers/specs/2026-05-20-planner-program-overview.md |
